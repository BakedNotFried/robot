...
class ImageRecorder:
    def __init__(
        ...
        self.camera_names = ['cam_overhead', 'cam_field', 'cam_wrist']

        for cam_name in self.camera_names:
            setattr(self, f'{cam_name}_image', None)
            setattr(self, f'{cam_name}_secs', None)
            setattr(self, f'{cam_name}_nsecs', None)
            if cam_name == 'cam_overhead':
                callback_func = self.image_cb_cam_overhead
            elif cam_name == 'cam_field':
                callback_func = self.image_cb_cam_field
            ...
            print(topic)
            node.create_subscription(Image, topic, callback_func, 1)
            if self.is_debug:
    ...
    def image_cb_cam_overhead(self, data):
        cam_name = 'cam_overhead'
        return self.image_cb(cam_name, data)

    def image_cb_cam_field(self, data):
        cam_name = 'cam_field'
        return self.image_cb(cam_name, data)
