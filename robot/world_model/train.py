...
for step in range(start_step, config.max_steps):
    ...
    model.train()
    optimizer.zero_grad()
    actions, q_pos, images, progress, next_images = train_loader.next_batch()
    actions = actions.to(device)
    q_pos = q_pos.to(device)
    images = images.to(device)
    progress = progress.to(device)
    # Trying to predict the next overhead image
    target = next_images.to(device)

    with torch.autocast(device_type=device.type, dtype=torch.bfloat16):
        # Infer the next frame
    ...
    if (step + 1) % config.validation_interval == 0 or last_step:
        ...
        with torch.no_grad():
            for i in range(config.validation_steps):
                actions, q_pos, images, progress, next_images = val_loader.next_batch()
                actions = actions.to(device)
                q_pos = q_pos.to(device)
                images = images.to(device)
                progress = progress.to(device)
